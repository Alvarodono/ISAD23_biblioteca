...
class LibraryController:
    ...
    def get_unread_books(self, user_email):
        """
        ...
        try:
            query = """
                   SELECT b.*
                   FROM Book b
                   LEFT JOIN User_Reserved_Book urb ON b.id = urb.book_id AND urb.user_email = ?
                   LEFT JOIN User_Read_Books urb2 ON b.id = urb2.book_id AND urb2.user_email = ?
                   WHERE urb.book_id IS NULL AND urb2.book_id IS NULL
               """
            unread_books_data = db.select(query, (user_email, user_email))

            unread_books = [
                Book(book_data[0], book_data[1], book_data[2], book_data[3], book_data[4])
                for book_data in unread_books_data
            ]

            return unread_books

        except Exception as e:
            print(f"Error al obtener libros no leídos: {e}")
            return []
